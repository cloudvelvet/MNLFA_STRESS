...
**문서 상태:** 논문 1 초안(v4) — 방법론적 제안 및 예비 실증 적용

---

## 국문초록

종단 연구에서 관찰점수의 변화가 잠재구성개념의 변화로 해석되려면 반복 문항이 시간과 개인 특성에 걸쳐 비교 가능한 방식으로 작동해야 한다. 그러나 이주배경 부모의 문화적응 스트레스 문항은 한국어 능력, 소득, 연령, 차별경험의 변화에 따라 역치 또는 변별도가 달라질 수 있으며, 이 경우 관찰된 응답 변화에는 잠재 스트레스 변화와 측정기능 변화가 함께 포함된다. 본 연구는 기존 response-shift 분해, MNLFA 및 비선형 반사실적 분해의 논리를 결합하여, 순서형 범주확률의 잠재·측정·상호작용 대비를 총변동거리로 요약하는 운용적 지표 RSDI를 제안하고 MAPS 2 부모 1–5차 자료에 예비 적용하였다. 분석자료는 2,191명, 9,662개 가구연결-시점, 77,296개 문항응답이며, 완전사례 필터 후 주모형에는 2,190개 가구연결 기록과 77,160개 응답이 포함되었다. 네 VI 시드에서 세 모형은 감소 방향을 보였으나 척도와 식별조건이 달라 절대크기는 비교하지 않았다. 차별 관련 threshold-DIF는 네 시드에서 같은 방향이었지만, 일부 기준 loading은 legacy hard-clamp 하한에 포화되었고 동일 structural-state NUTS는 완료되지 않았다. RSDI는 seed 20260702의 단일 VI 실행에서 .360, 문항 1·2·6 제외 한 번의 민감도 실행에서 .260으로 산출되었다. 두 값은 seed·앵커·경로·집계·잔차조건에 의존하는 탐색적 기술량이며 확증적 효과크기나 관찰변화의 백분율이 아니다. 단일요인 오지정, 차별 문항과 moderator의 의미중첩, 회상창 불일치, 가구 ID 기반 추적, 패널 이탈과 가중치 미적용을 해결하려면 앵커식별·다요인·동일모형 NUTS/PPC·W2–W5 민감도·IPW/공식 가중치·recovery simulation이 필요하다.

**주제어:** 문화적응 스트레스, 종단 측정불변성, 순서형 MNLFA, 차별경험, differential item functioning, response shift, RSDI

## Abstract

We propose the Response-Shift Decomposition Index (RSDI), an operational summary that combines established response-shift decomposition, moderated nonlinear factor analysis, and nonlinear counterfactual decomposition on the ordinal category-probability scale. We applied it preliminarily to Waves 1–5 of the MAPS 2 parent file. The derived file contained 2,191 household-linked records, 9,662 linked-record waves, and 77,296 item responses; complete-case filtering yielded 2,190 linked records and 77,160 responses. Four mean-field VI runs showed a common negative direction for discrimination-related parallel-threshold moderation, but baseline loadings reached the legacy hard-clamp boundary in some runs and matched structural-state NUTS was not completed. The RSDI was .360 in one seed-20260702 full-data VI run and .260 in one item-exclusion sensitivity run. These are path-, anchor-, aggregation-, and model-conditional exploratory summaries, not seed-averaged posterior estimates or validated proportions of observed change. The current analysis also leaves unresolved the single-factor versus multidimensional measurement structure, semantic overlap between discrimination and item content, non-equivalent recall windows, household-level rather than parent-level identifiers, panel attrition, survey weights, and posterior predictive validation. Anchor-identified matched-model NUTS/PPC, W2–W5 and weighted/IPW sensitivities, and recovery simulation are required before RSDI is interpreted as a validated effect-size index.
...
SEM 기반 response-shift 연구는 관찰변화를 잠재구성개념의 변화와 측정모수 변화로 분해해 왔으며, 이산·순서형 문항으로도 확장되었다(Oort, 2005; Verdam et al., 2016, 2021). MNLFA는 잠재평균·분산과 문항의 적재량·역치를 연속형 또는 범주형 공변량의 함수로 표현한다(Bauer, 2017). 비선형 반사실적 분해는 예측확률 차이를 특성 또는 모수 변화에 따른 성분으로 나누지만, 결과가 변수 교체 순서와 기준분포에 의존할 수 있음을 보여준다(Fairlie, 2017; Sinning et al., 2008). RSDI는 이 세 문헌을 대체하는 새로운 response-shift 이론이 아니라, 그 논리를 종단 순서형 MNLFA의 범주확률 공간에서 연결하는 운용적 지표이다.

두 시점 사이에서 기준 확률벡터, 잠재상태만 교체한 벡터, 측정공변량만 교체한 벡터, 둘을 함께 교체한 벡터를 구성하면 전체 확률변화 벡터를 잠재성분, 측정성분, 비가법적 상호작용성분으로 항등 분해할 수 있다. 순서범주의 간격을 가정하지 않기 위해 각 벡터 대비의 크기는 총변동거리(total variation distance)로 요약한다. 다만 벡터 항등식이 성립한다고 해서 각 성분의 총변동거리가 전체 총변동거리와 가산적으로 합쳐지는 것은 아니다. 절댓값을 취하는 과정에서 변화 방향과 성분 간 상쇄가 제거되기 때문이다.

따라서 RSDI는 설명분산, 인과적 매개비율 또는 관찰된 총변화의 백분율이 아니다. 선택한 모형, 식별제약, 공변량 교체경로, 시점쌍, 개인·문항 집계규칙에 조건부인 성분크기 정규화 지표이다. 본 연구의 주 RSDI는 연령, 한국어 능력, 소득, 차별경험의 측정경로를 함께 교체한 `RSDI_all-Z`이며, 차별경험 하나의 기여율이 아니다. 차별경험을 0에서 1로 바꾸는 고정-잠재 대비는 별도의 측정경로 대비로 보고한다.
...
따라서 이 연구의 현 추정치는 likelihood 수준에서 앵커 식별이 완성된 확증모형이 아니라 prior와 hard clamp가 정규화한 탐색적 해이다. 특히 ‘여덟 문항 모두 같은 방향의 threshold DIF’는 강한 보편적 DIF의 증거일 수도 있지만, 공통 잠재효과가 DIF 쪽으로 배분된 결과일 가능성도 배제할 수 없다. 최종 확증분석에서는 내용상 차별과 거리가 있는 복수 문항의 DIF를 0으로 두고 그중 한 marker 문항의 기준 적재량을 1로 고정한 뒤, 대체 앵커와 effects coding 민감도를 비교해야 한다. marker 적재량 고정과 잠재분산 고정을 동시에 적용하여 불필요한 중복 제약을 두지 않는다. legacy 실행의 기준 log-loading은 일부 시드에서 하드클램프 하한에 포화되어, 성장분산과 occasion 분산의 분해도 안정적이지 않았다. 이에 따라 본 개정판은 수치를 확증 결과로 승격하지 않고, 별도 maps_mnlfa_structural_state_anchored_v2.stan에 marker/복수 앵커, smooth bounded moderation, log-likelihood, posterior predictive replicate를 구현하여 후속 calibration NUTS의 대상으로 분리한다.
...
W1–W5 `RSDI_all-Z`는 한 seed의 조건부 탐색량으로 0.360이었다. 차별 관련 문항 1·2·6을 제외한 별도 단일 VI 실행에서는 0.260으로 감소하였다. 이 차이는 의미중첩 가설과 양립하지만, 문항구성과 잠재척도 식별이 동시에 달라졌으므로 문항내용의 순수한 영향으로 해석할 수 없다.
...
MAPS 2 부모 패널의 관찰평균과 두 잠재모형은 문화적응 스트레스가 평균적으로 감소하는 방향을 공통으로 보였다. 그러나 legacy structural-state MNLFA에서 잠재상태와 공통 DIF의 분리 식별이 충분하지 않았으므로, 감소의 절대크기와 측정성분의 상대크기는 확증적 추정치로 간주할 수 없다.
...
재현용 Stan·R 코드와 비식별 요약 산출물은 이 프로젝트의 `analysis/` 및 `outputs/`에 보존한다. 원자료는 자료이용계약상 재배포하지 않는다. 최종 투고 시에는 접근 가능한 코드 저장소, 소프트웨어 버전, 실행 명령, 분석 산출물 manifest와 합성 예제자료의 제공 범위를 명시한다.

## 연구윤리

본 연구는 기존 패널자료의 2차분석이다. 저자 소속기관의 연구윤리 심의면제 또는 승인 필요 여부와 해당 번호는 투고 전에 확인한다. 확인 전에는 본 원고를 제출용 확정본으로 간주하지 않으며, 원자료 제공기관의 조사승인 번호를 저자의 별도 연구윤리 승인번호로 전용하지 않는다.
