...
종단 연구에서 관찰점수의 변화가 잠재구성개념의 변화로 해석되려면 반복 문항이 시간과 개인 특성에 걸쳐 비교 가능한 방식으로 작동해야 한다. 그러나 이주배경 부모의 문화적응 스트레스 문항은 한국어 능력, 소득, 연령, 차별경험의 변화에 따라 역치 또는 변별도가 달라질 수 있으며, 이 경우 관찰된 응답 변화에는 잠재 스트레스 변화와 측정기능 변화가 함께 포함된다. 본 연구는 한국청소년정책연구원의 다문화청소년패널조사 2기(MAPS 2) 부모 1–5차 자료를 이용하여 종단 순서형 moderated nonlinear factor analysis(MNLFA)를 적용하고, 모형이 함의하는 범주확률 변화에서 잠재성분, 측정성분, 상호작용성분을 분리하는 Response-Shift Decomposition Index(RSDI)의 경험적 사용 가능성을 검토하였다. 파생 장기자료는 2,191명, 9,662개 개인-시점, 77,296개 문항응답으로 구성되었으며, 소득 공변량의 행 단위 완전사례 처리 후 주 MNLFA에는 2,190명과 77,160개 문항응답이 포함되었다. 비교분석으로 관찰점수 선형혼합모형과 측정불변 순서형 잠재성장모형을 추정하였다. 주모형은 개인별 잠재 절편·기울기, 개인-시점 공통 잔차, 잠재상태 공변량 효과, 문항별 적재량 및 평행 역치 DIF를 동시에 포함하였다. 네 개 초기값 시드의 평균장 변분추론 결과, 세 모형 모두 평균적인 감소 방향을 보였다. 관찰범주 평균은 1차 2.630에서 5차 2.437로 낮아졌고, 관찰점수 혼합모형의 시간기울기는 -0.042였다. 측정불변 순서형 모형의 잠재기울기는 -0.222, 주 MNLFA의 네 시드 평균은 -0.210이었으나, 서로 다른 척도와 식별조건 때문에 기울기의 절대크기는 직접 비교하지 않았다. 차별경험 관련 평행 역치 DIF는 여덟 문항 모두 네 변분 실행에서 음의 방향을 보였고, 각 실행의 90% 근사 변분구간이 0을 포함하지 않았다. 1차에서 5차까지 연령, 한국어 능력, 소득, 차별경험의 측정경로 변화를 함께 교체한 `RSDI_all-Z` 문항평균은 0.360(90% 근사 변분구간 [0.324, 0.396])이었다. 차별 내용과 가까운 문항 1, 2, 6을 제외하면 0.260([0.231, 0.334])으로 감소했고 근사 변분구간은 0을 포함하지 않았다. 이러한 결과는 문화적응 스트레스의 관찰 변화가 순수한 잠재변화만을 반영한다고 보기 어렵고, 차별경험 관련 측정기능 변화와 일관된 패턴이 존재할 가능성을 보여준다. 다만 현재의 공통 DIF–잠재상태 분리 문제, 평균장 변분추론의 불확실성 축소 가능성, 완전사례 처리와 패널 이탈, 조사 가중치 미적용 때문에 결과는 경험적 개념증명으로 제한된다. RSDI는 관찰변화의 인과적 비율이나 설명분산이 아니라 선택한 모형과 경로에서 측정성분 벡터의 상대적 크기를 요약한 지표이다.

**주제어:** MAPS 2, 다문화청소년 패널조사 2기, 문화적응 스트레스, 종단 측정불변성, 순서형 MNLFA, 차별경험, differential item functioning, response shift, RSDI

## Abstract

Interpreting longitudinal score change as change in an underlying construct requires repeated items to retain comparable measurement functions across occasions and respondent characteristics. This assumption may be especially fragile for acculturative-stress items because language proficiency, economic resources, age, and discrimination experiences can be associated not only with latent stress but also with the thresholds and discriminations that link stress to ordinal responses. We examined this problem using Waves 1–5 of the parent panel from the second Multicultural Adolescents Panel Study (MAPS 2) in Korea. The analysis combined a Bayesian longitudinal ordinal moderated nonlinear factor model with a model-implied Response-Shift Decomposition Index (RSDI). The derived long file contained 2,191 parents, 9,662 person-waves, and 77,296 item responses; row-level complete-case filtering on model covariates yielded 2,190 parents and 77,160 responses for the primary MNLFA. A naïve mixed model of observed mean categories and an invariant ordinal latent growth model served as descriptive benchmarks. Across four mean-field variational optimizations, all three models indicated average decline, although their slope magnitudes were not compared because the observed and latent scales differed. Discrimination-related parallel-threshold moderation was negative for all eight items in all four variational solutions, indicating a higher probability of endorsing elevated response categories at the same modeled latent state. For the Wave 1–Wave 5 contrast, the item-averaged `RSDI_all-Z`, which jointly replaced the measurement-path values of age, Korean proficiency, income, and discrimination, was .360 (90% approximate variational interval [.324, .396]). Excluding the three items most proximal to discrimination content attenuated the index to .260 [.231, .334]. These findings suggest that observed longitudinal change may contain a non-negligible measurement-function component. They do not establish a causal effect of discrimination or a validated proportion of change attributable to response shift. The current model lacks external DIF anchors, relies on prior-regularized mean-field variational inference, and has not been confirmed by full-data NUTS. The results therefore constitute an empirical proof of concept and motivate an anchor-identified confirmatory reanalysis.
...
MAPS 2 부모 패널의 문화적응 스트레스 척도는 차별대우, 편견, 향수, 낯선 환경, 무시, 위축, 낮은 사회적 지위 등 서로 다른 내용을 포함한다(한국청소년정책연구원, 2024, 「이용자를 위하여」 항목 5). 이 구성은 문화적응 스트레스의 다면성을 포착한다는 장점이 있지만, 차별경험 공변량과 일부 문항 내용의 의미적 중첩을 초래할 수 있다. 따라서 차별경험 관련 DIF가 나타나더라도 그것이 일반적인 응답기준 변화인지, 차별에 가까운 문항내용의 직접적 중첩인지, 또는 잠재상태와 측정경로를 분리하는 모형 식별의 산물인지 구분해야 한다.
...
표 1은 필터 이전 파생자료의 시점별 기술통계이다. 관찰된 부모 수는 1차 2,186명에서 5차 1,750명으로 감소하였다. 따라서 시간에 따른 평균 변화에는 개인내 변화뿐 아니라 패널 이탈과 시점별 표본구성 변화가 섞일 수 있다. 주 분석은 조사 가중치, 중도탈락 역확률가중치, 다중대치를 적용하지 않았다.
...
따라서 이 연구의 현 추정치는 likelihood 수준에서 앵커 식별이 완성된 확증모형이 아니라 prior와 hard clamp가 정규화한 탐색적 해이다. 특히 ‘여덟 문항 모두 같은 방향의 threshold DIF’는 강한 보편적 DIF의 증거일 수도 있지만, 공통 잠재효과가 DIF 쪽으로 배분된 결과일 가능성도 배제할 수 없다. 최종 확증분석에서는 내용상 차별과 거리가 있는 복수 문항을 사전 앵커로 지정하고, 기준시점 잠재평균 0·분산 1을 고정한 뒤 대체 앵커와 effects coding 민감도를 비교해야 한다.
...
분모가 `10⁻⁸` 이하인 경우 비율을 계산하지 않았다. 전체 `RSDI_all-Z`는 8개 문항별 RSDI의 산술평균이다. 이는 문항별 분자와 분모를 모두 합친 pooled-scale 비율이 아니다. 또한 `Z`의 교체는 연령, 한국어 능력, 소득, 차별경험의 관찰된 측정경로 변화를 함께 포함하므로, `RSDI_all-Z`를 차별경험 특정 지표로 해석하지 않았다.

본 분해는 잠재상태를 먼저 바꾸고 측정공변량을 바꾸는 한 경로에 기초한다. 변화 순서를 뒤집는 분해와 Shapley 대칭분해는 아직 구현하지 않았다. 또한 현재 RSDI는 시점쌍에서 모두 관찰된 사람의 교집합을 동일가중으로 사용하며, 중도탈락 또는 조사설계 가중치를 적용하지 않는다.
...
1차와 5차가 모두 관찰된 1,743명에 대해 연령, 한국어 능력, 소득, 차별경험의 측정경로를 함께 교체했을 때, 평균 잠재성분 총변동거리는 0.133, 측정성분은 0.096, 상호작용성분은 0.037이었다. 이 세 성분크기의 합을 분모로 한 8개 문항 `RSDI_all-Z`의 산술평균은 0.360이었고, 90% 근사 변분구간은 [0.324, 0.396]이었다.
...
W1–W5 `RSDI_all-Z`는 0.360이었고 문항 1·2·6 제외 시 0.260으로 감소하였다. 이 차이는 차별 관련 문항내용이 측정성분의 크기를 일부 증폭했음을 시사한다. 동시에 향수·낯선 환경을 다룬 문항 3–5뿐 아니라 외국출신 정체성과 사회적 대우가 여전히 포함된 문항 7–8에서도 threshold DIF와 고정-잠재 대비가 남았다. 이는 측정성분이 문항 1·2·6에만 전적으로 의존하지 않을 가능성을 보여주지만, 의미중첩이 완전히 제거됐다는 뜻은 아니다. 다만 이 결론은 한 번의 VI 민감도 분석에 근거하며, 앵커 변경이나 정확한 posterior 추정에 대한 강건성을 입증하지 않는다.

## 4.2 실질적 함의: 감소한 것은 무엇인가

관찰평균의 감소는 시간이 지남에 따라 부모가 높은 스트레스 범주를 덜 선택했다는 사실을 보여준다. 그러나 이 변화가 전적으로 잠재 문화적응 스트레스의 감소라고 단정하기는 어렵다. 언어능력, 소득, 연령, 차별경험의 분포가 변하고 이 변수들이 문항 역치와 관련되면, 같은 잠재 스트레스가 서로 다른 시점에 다른 범주로 표현될 수 있다. 종단 MNLFA의 핵심 장점은 이러한 측정경로를 잠재성장 과정과 함께 표현한다는 데 있다(Chen & Bauer, 2024, 2026).
...
MAPS 2 부모 패널에서 문화적응 스트레스의 관찰평균과 두 잠재모형은 모두 평균 감소 방향을 보였다. 동시에 prior-regularized structural-state MNLFA는 차별경험과 관련된 같은 방향의 평행 역치 이동을 산출했고, W1–W5 `RSDI_all-Z`는 측정성분의 크기가 무시하기 어려울 수 있음을 보여주었다. 차별 관련 문항 1·2·6을 제외하면 지표가 약화되었지만 완전히 사라지지는 않았다.

그러나 현재 결과가 확정하는 것은 ‘잠재 스트레스가 정확히 얼마나 감소했는가’ 또는 ‘차별이 response shift를 얼마나 일으켰는가’가 아니다. 현 자료와 모형에서 관찰된 종단 응답변화를 순수한 잠재변화로만 해석하는 것이 위험하며, 잠재경로와 측정경로를 분리하는 분석이 필요하다는 점을 보여준다. RSDI는 이 구분을 범주확률 공간에서 요약할 수 있는 유망한 도구이지만, 앵커 식별과 사후분포 검증, 결측·이탈 처리 및 경로 민감도에 대한 추가 평가가 필요하다.
...
본 연구는 한국청소년정책연구원에서 수행한 「다문화청소년 패널조사 2기(MAPS 2)」 데이터를 활용하였다. 자료는 한국청소년정책연구원의 이용조건과 개인정보 보호 규정에 따라 제공되며, 본 원고와 함께 개인수준 원자료를 재배포하지 않는다. 재현용 Stan·R 코드와 비식별 요약 산출물은 저널 정책과 자료이용계약이 허용하는 범위에서 공개할 예정이다.

## 연구윤리

MAPS 2 유저가이드에 따르면 원조사는 2019년부터 한국청소년정책연구원 기관생명윤리위원회 심의를 받아 수행되었다(한국청소년정책연구원, 2024, 「이용자를 위하여」 항목 5). 본 연구는 비식별 2차자료 분석이다. **저자 소속기관의 2차자료 연구 심의면제 또는 승인 여부와 번호는 최종 투고 전에 확인하여 이 문단에 입력해야 한다.** 원자료의 조사승인번호를 본 2차분석의 승인번호로 전용하지 않는다.
